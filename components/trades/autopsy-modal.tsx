...
export function AutopsyModal({ trade, onClose, onUpdated }: AutopsyModalProps) {
  const [loading, setLoading] = useState(false);
  const [result, setResult] = useState<AutopsyResult | null>(trade.autopsy_json ?? null);
  ...
  async function runAutopsy() {
    ...
    setError("");
    try {
      const res = await fetch("/api/trades/autopsy", {
        method: "POST",
  ...
  useEffect(() => {
    if (!result && !loading && !error) runAutopsy();
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="glass-strong relative w-full max-w-lg max-h-[85vh] rounded-2xl shadow-2xl shadow-black/40 overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex items-center justify-between border-b border-white/[0.06] px-5 py-3.5 shrink-0">
          <div>
            <h3 className="text-sm font-bold text-foreground">Trade Autopsy</h3>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-sm font-medium text-foreground">Analyzing your trade...</p>
              <p className="text-[11px] text-muted-foreground">Reviewing entry, exit, and execution</p>
            </div>
          )}
          ...
          {error && !loading && (
            <div className="text-center py-8 space-y-3">
              ...
              <button onClick={runAutopsy} className="rounded-lg bg-white/[0.06] px-4 py-2 text-xs text-foreground hover:bg-white/[0.1]">Retry</button>
            </div>
          )}
          ...
          {result && !loading && (
            <>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">What Went Well</p>
                  </div>
                ...
                <div>
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">What Went Wrong</p>
                  </div>
                  <ul className="space-y-1.5">
              ...
              <div className="flex items-center justify-between pt-2 border-t border-white/[0.06]">
                ...
                <button onClick={runAutopsy} disabled={loading} className="inline-flex items-center gap-1 rounded px-2 py-1 text-[10px] text-muted-foreground hover:text-foreground hover:bg-white/[0.06] disabled:opacity-50">
